...
function watch() {
  gulp.watch(resolvePath(paths().source.css, '**/*.scss')).on('change', gulp.series('pl-sass'));
  console.log(resolvePath(paths().source.css, '**/*.scss'));
  gulp.watch(resolvePath(paths().source.css) + '/**/*.css', { awaitWriteFinish: true }).on('change', gulp.series('pl-copy:css', reloadCSS));
  console.log(resolvePath(paths().source.css) + '/**/*.css');
...
/******************************************************
 * Multi site setup tasks
******************************************************/
//read all paths from our namespaced config file
var starterkitConfig = require('./starterkit-config.json');

function starterkitPaths() {
  return starterkitConfig.starterkitUpdatePaths;
}

gulp.task('updatestarterkit', function(updateCompleted) {
  var starterkitFoldersList = starterkitPaths();
  var sourceFolder = '';
  var destFolder = '';

  // Copy pattern folders
  for (i=0; i<starterkitPaths().source.patternFolders.length; i++) {
    sourceFolder = resolvePath(starterkitPaths().base + starterkitPaths().source.patterns + starterkitPaths().source.patternFolders[i]);
    destFolder = resolvePath(paths().source.patterns + starterkitPaths().source.patternFolders[i]);

    console.log('Copying ' + sourceFolder + ' to ' + destFolder);
  ...
  // Copy assets folders
  for (i=0; i<starterkitPaths().source.assets.length; i++) {
    sourceFolder = resolvePath(starterkitPaths().base + starterkitPaths().source.root + starterkitPaths().source.assets[i]);
    destFolder = resolvePath(paths().source.root + starterkitPaths().source.assets[i]);
